...
% By Evan Bollig (2012)
% Based on matrix market mmread, but adds support for BINARY content for
% Coordinate matrices as (int,int,double).
%
...
if ( strcmp(rep,'coordinate')) %  read matrix given in sparse
    ...
    if  ( strcmp(field,'real') )               % real valued entries:
        ...
        T=[];
        for inefficient = 1:entries
         ineff_row = fread(mmfile,1,'int');
         ineff_col = fread(mmfile,1,'int');
         ineff_val = fread(mmfile,1,'double');
         T = [T; ineff_row, ineff_col, ineff_val];
        end
        %T = [T; fscanf(mmfile,'%f')];
        
        if ( size(T,2) ~= 3 ) & ( size(T,1) ~= entries )
            message = ...
...
%         T = reshape(T,3,entries)';
    ...
    elseif   ( strcmp(field,'complex'))            % complex valued entries:
        ...
        %
        error('No support for complex/pattern')
